...
class CGAN_mnist(L.LightningModule):
    ...
    def training_step(self, batch):
        ...
        ## ----------------------------------------------------------------
        ## Train discriminator using real and then fake FashionMNIST images,
        ...
        for _ in range(self.k):
          # Real MNIST images
          d_real_logits_out = self.discriminator(imgs,labels)
          d_real_loss = self.real_loss(d_real_logits_out, self.loss_fn, device)
          # Fake images
          with torch.no_grad():
            # Generate a batch of random latent vectors
            z = np.random.uniform(-1, 1, size=(imgs.size(0), self.latent_dim))
            z = torch.from_numpy(z).float().to(device)
            # Generate batch of fake images
            fake_labels = Variable(torch.LongTensor(np.random.randint(0, 10, imgs.size(0))))
            fake_labels = fake_labels.to(device)
            fake_images = self(z,fake_labels)
          # feed fake-images to discriminator and compute the
          # fake_loss (i.e. target label = 0)
          d_fake_logits_out = self.discriminator(fake_images,fake_labels)
        ...
        # Generate a batch of random latent vectors
        z = np.random.uniform(-1, 1, size=(imgs.size(0), self.hparams.latent_dim))
        z = torch.from_numpy(z).float().to(device)
        ...
        fake_labels = fake_labels.to(device)
        fake_images = self(z,fake_labels)
        g_logits_out = self.discriminator(fake_images,fake_labels)
